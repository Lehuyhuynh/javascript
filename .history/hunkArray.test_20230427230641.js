...
test(' The longest increasing subsequence is ', () => {
  ...
  const sortedArr =  rearrangingthecharacters(myArray,3);
  expect(sortedArr).toEqual(4)
});
test(' The longest increasing subsequence is ', () => {
  const myArray = [3,2,6,3,7,8,5];
  const sortedArr =  rearrangingthecharacters(myArray,2);
  expect(sortedArr).toEqual(4);
});
test(' The longest increasing subsequence is ', () => {
  const myArray =[-3,-4,5,26,3]
  const sortedArr =  rearrangingthecharacters(myArray,1);
  expect(sortedArr).toEqual(3);
});
test(' The longest increasing subsequence is ', () => {
  const myArray = [3,6,2,9]
  const sortedArr = rearrangingthecharacters(myArray,);
  expect(sortedArr).toEqual(3);
});
