const  characters= require("./charactersearch");

test("đếm số chuỗi có kí tự a trong 1 list strings", () => {
  expect(characters(["avbc", "bbxxcvv", "asadfqw", "ccsaff"])).toBe(3);
});

test("đếm số chuỗi có kí tự a trong 1 list strings", () => {
  expect(characters(["asd", "a", "bbsad", "drrar"])).toBe(4);
});

test("đếm số chuỗi có kí tự a trong 1 list strings", () => {
  expect(characters(["sd", "", "bbsd", "drrr"])).toBe(0);
});

test("đếm số chuỗi có kí tự a trong 1 list strings", () => {
  expect(characters(["sd", "ad", "bbsd", "drrr", "dhdshas"])).toBe(2);
});
