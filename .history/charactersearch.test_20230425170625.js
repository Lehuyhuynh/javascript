const  ch= require("./charactersearch");

test("đếm số chuỗi có kí tự a trong 1 list strings", () => {
  expect(ch(["avbc", "bbxxcvv", "asadfqw", "ccsaff"])).toBe(3);
});

test("đếm số chuỗi có kí tự a trong 1 list strings", () => {
  expect(ch(["asd", "a", "bbsad", "drrar"])).toBe(4);
});

test("đếm số chuỗi có kí tự a trong 1 list strings", () => {
  expect(ch(["sd", "", "bbsd", "drrr"])).toBe(0);
});

test("đếm số chuỗi có kí tự a trong 1 list strings", () => {
  expect(ch(["sd", "ad", "bbsd", "drrr", "dhdshas"])).toBe(2);
});
